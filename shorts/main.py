...
def run_pipeline(json_path: str, stories_dir: str, stages: list[int]) -> dict:
    ...
    try:
        ...
        if 2 in stages:
            print("[2/5] Structuring story with Phi-3-mini...")
...
def main():
    ...
    parser.add_argument("--cases-dir", default=CASES_DIR, help="Folder with case JSON files")
    parser.add_argument("--stories-dir", default=STORIES_DIR, help="Folder to save story JSONs")
    parser.add_argument("--stages", default="1,2", help="Comma-separated stages to run e.g. 1,2,3,4,5")
    parser.add_argument("--file", default=None, help="Process a single JSON file instead of whole folder")
    parser.add_argument("--force", action="store_true", help="Re-process already completed files")
    ...
    for json_path in files:
        slug = Path(json_path).stem
        if not args.force and already_processed(slug, args.stories_dir):
